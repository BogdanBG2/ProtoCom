...
void ping(int sockfd, struct in_addr ip_addr, int count)
{
	...
	for(int i = 0; i < count; ++i) {
		...
		ip_hdr->tot_len = pkt.size - sizeof(struct ether_header);
		...
		ip_hdr->daddr = ip_addr.s_addr;
		ip_hdr->check = 0;
		ip_hdr->check = checksum(ip_hdr, sizeof(ip_hdr)); // pointer at the IP header
		...
		icmp_hdr->un.echo.id = htons(getpid() & 0xFF);
		icmp_hdr->checksum = 0;
		icmp_hdr->checksum = checksum(icmp_hdr, sizeof(icmp_hdr)); // pointer at the ICMP header
